...
void CAdaBNO055::GetSensor( TSensor* sensorOut )
{
	// Clear the sensor object
	memset( sensorOut, 0, sizeof( TSensor ) );

	// Insert the sensor's name
	strncpy( sensorOut->name, "BNO055", sizeof( sensorOut->name ) - 1 );

	sensorOut->name[sizeof( sensorOut->name ) - 1] = 0;
	sensorOut->version     = 1;
	sensorOut->sensor_id   = m_sensorId;
	sensorOut->type        = SENSOR_TYPE_ORIENTATION;
	sensorOut->min_delay   = 0;
	sensorOut->max_value   = 0.0F;
	sensorOut->min_value   = 0.0F;
	sensorOut->resolution  = 0.01F;
}

bool CAdaBNO055::GetEvent( TSensorEvent* eventOut )
{
	// Clear the event object
	memset( eventOut, 0, sizeof( TSensorEvent ) );

	eventOut->version   = sizeof( TSensorEvent );
	eventOut->sensor_id = m_sensorId;
	eventOut->type      = SENSOR_TYPE_ORIENTATION;
	eventOut->timestamp = millis();

	// Get a Euler angle sample for orientation
	imu::Vector<3> euler;
	bool ret = GetVector( VECTOR_EULER, euler );
	eventOut->orientation.x = euler.x();
	eventOut->orientation.y = euler.y();
	eventOut->orientation.z = euler.z();
