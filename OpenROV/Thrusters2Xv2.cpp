  ...
  if (command.cmp("lift") || command.cmp("strafe")  ){
    ...
    if (command.cmp("strafe")){
      if (command.args[1]>=-100 && command.args[1]<=100) {
        ...
        maxVtransDelta = abs((500.0/abs(port_vertical_motor.motor_negative_modifer))*(1.0-abs(trg_lift)));
        Serial.print("THR2XV2.mvd:");Serial.print(maxVtransDelta);Serial.println(";");
        st = constrain( (500.0/abs(port_vertical_motor.motor_negative_modifer))*trg_strafe,-maxVtransDelta,maxVtransDelta);
