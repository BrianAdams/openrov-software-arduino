...
/*
On the OpenROV we can push this to 50Hz max for gyro/accelermoter.
At 25Hz we use about 1/5 the available compute power on the AtMega.
*/
...
#include "Timer.h"
//#include "kriswinerquaternionFilters.h"
#include <Wire.h>
#include <Arduino.h>
...
#define ADO 0
#if ADO
static byte MPU9150_ADDRESS = 0x69;  // Device address when ADO = 1
#else
static byte MPU9150_ADDRESS = 0x68;  // Device address when ADO = 0
...
static signed char imu_orientation[9] = { 0, -1, 0,
                                           -1, 0, 0,
                                           0, 0, -1};

void MPU9150_DIRECT::device_setup()
{
...
//  delay(100);
  ...
  Serial.print("MPU9150 should be:");Serial.print(MPU9150_ADDRESS);Serial.print("=");Serial.print(c);Serial.println(";");

  if (!((c == 0x68 ) || (c == 0x69)) ) // WHO_AM_I should always be 0x68
  {
    if (MPU9150_ADDRESS == 0x68) {
      MPU9150_ADDRESS = 0x69;
    } else {
  ...
  Serial.println("MPU9150: online;");

  MPU6050SelfTest(SelfTest);
  if(!(SelfTest[0] < 1.0f && SelfTest[1] < 1.0f && SelfTest[2] < 1.0f && SelfTest[3] < 1.0f && SelfTest[4] < 1.0f && SelfTest[5] < 1.0f)) {
    Serial.println("MPU9150: failed self test;");
    return;
  }
  calibrateMPU9150(gyroBias, accelBias); // Calibrate gyro and accelerometers, load biases in bias registers
  Serial.println("MPU9150: calibrated;");

  Serial.print(F("MPU9150.abias.x:"));Serial.print((int)(1000*accelBias[0]));Serial.println(';');
  Serial.print(F("MPU9150.abias.y:"));Serial.print((int)(1000*accelBias[1]));Serial.println(';');
  Serial.print(F("MPU9150.abias.z:"));Serial.print((int)(1000*accelBias[2]));Serial.println(';');


  Serial.print(F("MPU9150.gbias.x:"));Serial.print(gyroBias[0]);Serial.println(';');
  Serial.print(F("MPU9150.gbias.y:"));Serial.print(gyroBias[1]);Serial.println(';');
  Serial.print(F("MPU9150.gbias.z:"));Serial.print(gyroBias[2]);Serial.println(';');

  initMPU9150(); // Inititalize and configure accelerometer and gyroscope
  Serial.println("MPU9150: initialized;");

  writeByte(AK8975A_ADDRESS, AK8975A_CNTL, 0x00); // powerdown
  delay(100);

  // Read the WHO_AM_I register of the magnetometer, this is a good test of communication
  c = readByte(AK8975A_ADDRESS, WHO_AM_I_AK8975A);  // Read WHO_AM_I register for AK8975A
  Serial.print("MPU9150.Mag should be 0x48:");Serial.print(c,HEX);Serial.print(";");

  // Get magnetometer calibration from AK8975A ROM
  initAK8975A(magCalibration);
  Serial.print(F("MPU9150.mag.sense.x:"));Serial.print(magCalibration[0]);Serial.println(';');
  Serial.print(F("MPU9150.mag.sense.y:"));Serial.print(magCalibration[1]);Serial.println(';');
  Serial.print(F("MPU9150.mag.sense.z:"));Serial.print(magCalibration[2]);Serial.println(';');

  MagRate = 20; // set magnetometer read rate in Hz; 10 to 100 (max) Hz are reasonable values
  magtimer.reset();
  sampletimer.reset();
  setup_successful = true;
}

void MPU9150_DIRECT::device_loop(Command command)
{
  if (!setup_successful){
    device_setup();
    return;
    ...
    if(!sampletimer.elapsed(1000/40)){
      return;
    }
    ...
    //The below this statement request the data, this just checks to see
    //if it is ready for processing.
    if (readMagData(magCount)){  // Read the x/y/z adc values
    ...
    if (magtimer.elapsed (1000/MagRate))
    {

      byte c = readByte(AK8975A_ADDRESS, WHO_AM_I_AK8975A);  // Read WHO_AM_I register for AK8975A
      Serial.print("MPU9150.Mag should be 0x48:");Serial.print(c,HEX);Serial.print(";");
      if (c != 0x48) {
        Serial.print("MPU9150.Mag:failed;");
        writeByte(AK8975A_ADDRESS, AK8975A_CNTL, 0x00); // powerdown
      } else {
        Serial.print("MPU9150.Mag:online;");
        requestMagData();
      }
    }
  ...
  //rotate the vectors
  ax = imu_orientation[0] * ax + imu_orientation[1] * ax + imu_orientation[2] * ax;
  ay = imu_orientation[3] * ay + imu_orientation[4] * ay + imu_orientation[5] * ay;
  az = imu_orientation[6] * az + imu_orientation[7] * az + imu_orientation[8] * az;

  gx = imu_orientation[0] * gx + imu_orientation[1] * gx + imu_orientation[2] * gx;
  gy = imu_orientation[3] * gy + imu_orientation[4] * gy + imu_orientation[5] * gy;
  gz = imu_orientation[6] * gz + imu_orientation[7] * gz + imu_orientation[8] * gz;

  mx = imu_orientation[0] * mx + imu_orientation[1] * mx + imu_orientation[2] * mx;
  my = imu_orientation[3] * my + imu_orientation[4] * my + imu_orientation[5] * my;
  mz = imu_orientation[6] * mz + imu_orientation[7] * mz + imu_orientation[8] * mz;
...
// MahonyQuaternionUpdate(ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f, my, mx, mz);
    ...
    Serial.print("MPU9150.ax:"); Serial.print((int)1000*ax); Serial.println(';');
    Serial.print("MPU9150.ay:"); Serial.print((int)1000*ay);Serial.println(';');
    Serial.print("MPU9150.az:"); Serial.print((int)1000*az); Serial.println(';');
    Serial.print("MPU9150.gx:"); Serial.print( gx, 2);Serial.println(';');
    Serial.print("MPU9150.gy:"); Serial.print( gy, 2);Serial.println(';');
    Serial.print("MPU9150.gz:"); Serial.print( gz, 2); Serial.println(';');
    Serial.print("MPU9150.mx:"); Serial.print( (int)mx );Serial.println(';');
    Serial.print("MPU9150.my:"); Serial.print( (int)my );Serial.println(';');
    Serial.print("MPU9150.mz:"); Serial.print( (int)mz ); Serial.println(';');

    Serial.print("MPU9150.q0:"); Serial.print(q[0]);Serial.println(';');
    Serial.print("MPU9150.qx:"); Serial.print(q[1]);Serial.println(';');
    Serial.print("MPU9150.qy:"); Serial.print(q[2]);Serial.println(';');
    Serial.print("MPU9150.qz:"); Serial.println(q[3]);Serial.println(';');
  ...
  yaw = atan2(2 * q[1] * q[2] - 2 * q[0] * q[3], 2 * q[0]*q[0] + 2 * q[1] * q[1] - 1) * 180/PI;
  pitch = atan(grx / sqrt(gry*gry + grz*grz))  * 180/PI;
  roll = atan(gry / sqrt(grx*grx + grz*grz))  * 180/PI;
...
//    navdata::HDGD = 360-navdata::HDGD;
    float field = sqrt(sq(mx) + sq(my) + sq(mz));
    Serial.print(F("mag.field:")); Serial.print(field/10.0); Serial.print(";"); //uT
